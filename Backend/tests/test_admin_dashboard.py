...
from app.auth import CurrentUser, OrganizationAccess, require_owner
...
class AdminDashboardTests(unittest.TestCase):
    ...
    def test_owner_dependency_allows_only_owner_memberships(self):
            ...
            with self.subTest(role=role):
                ...
                if expected_status:
                    with self.assertRaises(HTTPException) as raised:
                        require_owner(organization_id=ORGANIZATION_ID, user=self.user, db=db)
                    self.assertEqual(raised.exception.status_code, expected_status)
                else:
                    access = require_owner(organization_id=ORGANIZATION_ID, user=self.user, db=db)
    ...
    def test_owner_response_contains_only_requested_organization_data(self):
        ...
        db = MagicMock()
        db.query.side_effect = [organization_query, membership_query]
